...
class OpenSearchServerlessConfig(BaseModel):
    ...
    def validate_config(cls, values: dict) -> dict:
        if not values.get("host"):
            raise ValueError("config OPENSEARCH_SERVERLESS_HOST is required")
        if not values.get("port"):
            raise ValueError("config OPENSEARCH_SERVERLESS_PORT is required")
        return values
...
class OpenSearchServerlessVector(BaseVector):
    ...
    def delete_by_ids(self, ids: list[str]) -> None:
        ...
        actions = [{"_op_type": "delete", "_index": index_name, "_id": es_id} for es_id in ids]
        try:
            helpers.bulk(self._client, actions)
...
class OpenSearchServerlessVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> OpenSearchServerlessVector:
        ...
        open_search_config = OpenSearchServerlessConfig(
            host=dify_config.OPENSEARCH_SERVERLESS_HOST or "localhost",
            port=dify_config.OPENSEARCH_SERVERLESS_PORT,
            user=dify_config.OPENSEARCH_SERVERLESS_USER,
            password=dify_config.OPENSEARCH_SERVERLESS_PASSWORD,
        )

        return OpenSearchServerlessVector(collection_name=collection_name, config=open_search_config)
